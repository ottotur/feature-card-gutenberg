...
// Configuration
const SHARED_COLORS_PATH = path.resolve(__dirname, '../../src/colors.css');
...
/**
 * Verify shared colors.css exists
 */
function verifyColorsFile() {
  console.log('🔍 Checking for shared colors.css...');
  
  if (!fs.existsSync(SHARED_COLORS_PATH)) {
    console.error(`❌ Error: Shared colors.css not found at ${SHARED_COLORS_PATH}`);
    console.error('   Expected location: feature-card-gutenberg/src/colors.css');
    process.exit(1);
  }
  
  console.log('✅ Found shared colors.css');
}
 ...
 */
function main() {
  console.log('🚀 Generating theme.json from shared colors...\n');
  
  try {
    // Step 1: Verify shared colors.css exists
    verifyColorsFile();
    
    // Step 2: Parse colors from shared file
    const cssContent = fs.readFileSync(SHARED_COLORS_PATH, 'utf8');
    ...
    console.log('\n✨ theme.json generation complete!');
  } catch (error) {
